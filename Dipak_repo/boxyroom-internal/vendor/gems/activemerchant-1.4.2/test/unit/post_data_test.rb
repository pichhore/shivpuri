...
class MyPost < PostData
  self.required_fields = [ :ccnumber, :ccexp, :first_name, :last_name, :username, :password, :order_id, :key, :time ]
end
...
class PostDataTest < Test::Unit::TestCase
  ...
  def test_subclass
    post = MyPost.new
    assert_equal [ :ccnumber, :ccexp, :first_name, :last_name, :username, :password, :order_id, :key, :time ], post.required_fields
  end
end