class Array
  ...
  #
  # Support the new jsonify plugin by modifying Array.to_json to support passing the options
  # to each element and collecting them into a JSON array format
  #
  def to_json(options = {})
    return "[" + self.collect { |el| el.to_json(options)}.join(",") + "]"
  end
