module Ym4r
  module GmPlugin
    ...
    class GMap
      ...
      def initialize(container, draggable_cursor = "pointer", variable = "map")
        @container = container
        @variable = variable
      ...
      def to_html(options = {})
        ...
        # JWH: Begin
        # html << "startTime = new Date().valueOf();"
        # BRD: Temporary until we can get the loading box centered and disappearing
        # html << "$$('div.loading span.text').each(function(item) {item.update('Preparing Map...');});"
        # html << "$$('div.loading').each(function(item) {item.show();});"
        # html << ""
        # BRD: End
        # JWH: End
        ...
        html << "GEvent.addListener(map, \"load\", addMapLoadingReport);\n"
        html << @init_begin * "\n"
        html << @init * "\n"
        html << @init_end * "\n"
        # JWH: Begin
        # html << "endTime = new Date().valueOf();"
        # html << "$$('#loading span.text')[0].update('Requesting Map From Google...');"
        # html << "$('loading').show();"
        # html << "alert(endTime-startTime);"
        # JWH: End
        
        # The small map copyright sticks out and obscures text on IE

        html << "setTimeout(function() {"
        html << "  try {"
        html << "   var divs = $$('div.gmnoprint');"
        html << "   if (divs && divs.length > 0) {"
        html << "     for (var i=0; i < divs.length; ++i) {"
        html << "       if (divs[i].innerHTML.indexOf('Terms of Use')) {"
        html << "         if (navigator.appVersion.match(/\bMSIE\b/))"
        html << "           Element.setStyle(divs[i], {'overflow':'hidden'});"
        html << "       }"
        html << "     }"
        html << "   }"
        html << "  } catch (ex) {}"
        ...
        if streetview then
            html << "\nstreetview_controller = new NL.StreetViewController(map);\n"
        end
      ...
      def create
        ...
        # standard cursors: auto, crosshair, default, help, move, pointer, text, wait
        # plus (browser dependent): hand, progress, not-allowed, no-drop, vertical-text, all-scroll
        # "new GMap2(document.getElementById(\"#{@container}\"), {draggableCursor: 'pointer', draggingCursor: 'move'})"
        "new GMap2(document.getElementById(\"#{@container}\"), {draggableCursor: \"#{@draggable_cursor}\"})"
        #MJW: End
      end
    end
  end
