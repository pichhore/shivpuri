    ...
    class QueryBase
        ...
        def results
            ...
            for cls, ids in lhash
                ...
                for f in found
                    chash[[cls, f.id]] = f
										class_uses_string_key[cls] = class_uses_string_key[cls] || f.id.kind_of?(String)
                end
            end
            # now get them in right order again
            results = []
            docs.each{|doc| 
								cls, id = doc[:data].split('-', 2); 
								results << { :model => chash[[cls, (class_uses_string_key[cls] ? id : id.to_i)]],
