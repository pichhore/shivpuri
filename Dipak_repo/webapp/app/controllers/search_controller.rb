class SearchController < ApplicationController
  ...
  def show
    ...
    @profiles.each { |profile|
      if profile && !profile.zip_code.blank?
        zip = Zip.find(:first, :conditions => ['zip = ?', profile.zip_code.split(",")[0]])
      end
  ...
  def build_query_string(params)
    ...
    query_string = "(status:true) " + query_string
    query_string = "(ownerprofile:true) " + query_string if search_owner
