class ProfileImagesController < ApplicationController
  ...
  def create
    pi = ProfileImage.find_by_profile_id(@profile.id)
    image_count =0
    puts params.inspect
    ...
    if !property.nil? && pi.nil? && image_count != 0
      property.post_property_to_twitter(current_user) if current_user && current_user.twitter
      property.post_property_to_facebook(current_user) if current_user && !current_user.fb_access_token.blank?
    end
