class UserCompanyImage < ActiveRecord::Base
  ...
  def find_profile_thumbnail
    UserCompanyImage.find(:first, :conditions=>["parent_id = ? and thumbnail = 'profile'", self.id])
  end
  
  def self.find_all(user)
    self.find_all_by_user_id(user)
  end

  def find_medium_thumbnail
    UserCompanyImage.find(:first, :conditions=>["parent_id = ? and thumbnail = 'medium'", self.id])
  end

  def find_tiny_thumbnail
    UserCompanyImage.find(:first, :conditions=>["parent_id = ? and thumbnail = 'tiny'", self.id])
  end
...
# We handle our own thumbnail generation
  ...
  def self.find_company_profile_image(current_user_id)
